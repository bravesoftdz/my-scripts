...
# direccion ip del server y opciones, por ej el puerto ssh en este caso 2022 en lugar de 22
...
# elimino archivo de registro de error
rm -f $0.err

# elimino flag del server
ssh $SSH_HOST $SSH_OPTS rm -f $FLG_NAME 
...
# activo flag
ssh $SSH_HOST $SSH_OPTS touch $FLG_NAME

# borro cualquier rastro del directorio temporal destino
ssh $SSH_HOST $SSH_OPTS rm -rf $DES_PATH/backup

# realizo backup a directorio destino backup (creo previamente los directorios necesarios)
echo $(date) generando backup >> $LOGFILE
ssh $SSH_HOST $SSH_OPTS mkdir -p $DES_PATH/backup/$DES_NAME
ssh $SSH_HOST $SSH_OPTS mkdir -p $DES_PATH/backup.0/$DES_NAME
rsync -a -e "ssh $SSH_OPTS" --compress --delete --stats --quiet --log-file-format="" --log-file=$LOGFILE $EXCLUDE --link-dest=$DES_PATH/backup.0/$DES_NAME $SRC_PATH/ $SSH_HOST:$DES_PATH/backup/$DES_NAME >> $0.err
...
if [ ! -s "$0.err" ]; then
  ...
  ssh $SSH_HOST $SSH_OPTS touch $DES_PATH/backup/$CRT_WEEK
  ssh $SSH_HOST $SSH_OPTS touch $DES_PATH/backup/$CRT_DATE
  ssh $SSH_HOST $SSH_OPTS touch $DES_PATH/backup/$CRT_TIME
